...
function setMode(){
  for(var i=0; i<mode.length; i++){
    mode[i].addEventListener("click", function(){
      ...
      if (this.textContent === "*Легкий*"){size = 3}
      else if (this.textContent === "*Сложный*"){size = 6}
      else {size = 9}
      //this.textContent === "Легкий" ? size = 6 : size = 9;
      resetColors();
    });
  }
}
  ...
  for(var i=0; i < squares.length; i++){
    squares[i].addEventListener("click", function(){
      ...
      if (clickedColor === pickedColor){
        mes.textContent = "Правильно!";
        reset.textContent = "Играть еще раз?";
        changeColor(pickedColor);
        document.querySelector("header").style.backgroundColor = pickedColor;
      } else {
        this.style.backgroundColor = document.body.style.backgroundColor;
        mes.textContent = "Это был " + clickedColor + ". Попробуй еще раз!";
      }
    });
  }
}